                    ...
                    for (var prop in obj) {
                        if (prop != "Input") {
                            ...
                            for (var prop2 in obj[prop]) {
                                if (prop2 == null || typeof prop2 == "undefined")
                                    continue;
                                else {
                                    //Find an element with class, id, name, or rel attribute that matches the property name
                                    var child = jQuery.makeArray(jQuery('[name="Input[' + prop2 + ']"]', element).length > 0 ? jQuery('[name="Input[' + prop2 + ']"]', element) :
                                        jQuery('[rel="' + prop2 + '"]'));
                                    if (child.length != 0) {
                                        var obj2 = obj[prop];
                                        bind(obj2[prop2], jQuery(child, element), `Input[${prop2}]`);
                                    } else {
                                        continue;
                                    }
                                }
